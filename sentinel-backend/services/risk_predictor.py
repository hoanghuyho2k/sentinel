...
def predict_risk_score(features: dict):
    if not MODEL_LOADED:
        ...
        return {"risk_score": score, "factors": features, "message": f"Risk score (heuristic): {score}%"}
    import pandas as pd
    row = pd.DataFrame([features])
    # dmat = xgb.DMatrix(row)
    pred = MODEL.predict(dmat)[0]
    score = float(pred) * 100.0
    return {"risk_score": round(score,2), "factors": features, "message": f"Risk score (model): {score:.2f}%"}
